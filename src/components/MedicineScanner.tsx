        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[80] bg-black/50 flex items-end sm:items-center justify-center p-4" onClick={() => { reset(); onClose(); }}>
          <motion.div initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }} transition={{ type: 'spring' }} className="glass-card w-full max-w-md p-5" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <p className="font-bold text-slate-800 dark:text-white flex items-center gap-2"><Scan size={18} className="text-primary-600" /> Scan Pill / Prescription</p>
            ...
            {!photo && (
              <button onClick={() => fileRef.current?.click()} className="w-full border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl py-10 flex flex-col items-center gap-2 text-slate-400 hover:border-primary-400 hover:text-primary-500 transition">
                <Camera size={36} />
                <span className="text-sm font-medium">Upload or snap a photo of the medication label</span>
              </button>
            )}
            ...
            {result && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-3 p-4 rounded-xl bg-success-50 dark:bg-success-700/20 border border-success-200 dark:border-success-700">
                <p className="text-xs font-bold text-success-700 dark:text-success-400 mb-2 flex items-center gap-1"><Check size={14} /> Scan Complete — Auto-filled</p>
                <div className="space-y-1.5 text-sm">
                  <Row label="Medication" value={result.name} />
                  <Row label="Dosage" value={result.dosage} />
                  <Row label="Frequency" value={result.time} />
                  <Row label="Instructions" value={result.instructions} />
                  <Row label="Expiration" value={result.expiration} />
                </div>
              </motion.div>
            )}

            {photo && !scanning && !result && (
              <Button className="w-full mb-2" onClick={scan}><Scan size={16} /> Run AI Scan</Button>
            )}
            {result && (
              <Button className="w-full mb-2" onClick={useResult}><Pill size={16} /> Add to Medications</Button>
            )}
            {photo && (
              <Button variant="ghost" className="w-full" onClick={() => fileRef.current?.click()}><Upload size={16} /> New Photo</Button>
            )}
          </motion.div>
        </motion.div>
      )}
