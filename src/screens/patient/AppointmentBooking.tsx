...
const SPECIALTIES = ['All', 'Orthopedics', 'Cardiology', 'Post-Op Care', 'General Surgery'];
...
export function AppointmentBooking() {
  ...
  const confirm = () => {
    ...
    setState(s => {
      ...
      patients[0] = { ...patients[0], nextAppointment: { doctor: selectedDoctor.name, date: dateLabel, time: formatTime(slot), type: type === 'telehealth' ? 'Video Call' : 'In-Person Visit' } };
      return { ...s, patients };
    });
    setConfirmed(true);
    show({ type: 'success', title: 'Appointment Confirmed', body: `${selectedDoctor.name} on ${dateLabel} at ${formatTime(slot)}` });
  };
  ...
  return (
    <div className="pb-24 px-4 pt-6 max-w-md mx-auto">
      ...
      <PageHeader title="Book Appointment" subtitle="Find a doctor and schedule a visit" />
      ...
      <AnimatePresence mode="wait">
        {confirmed ? (
          <motion.div key="conf" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card p-6 text-center">
            ...
            <p className="font-extrabold text-lg text-slate-800 dark:text-white mb-1">Appointment Confirmed!</p>
            <p className="text-sm text-slate-500 mb-4">{selectedDoctor?.name} · {DATES.find(d => d.key === date)?.label} at {formatTime(slot || '')}</p>
            <div className="flex gap-3">
              <Button variant="ghost" className="flex-1" onClick={reset}>Book Another</Button>
              <Button className="flex-1" onClick={() => back()}>Done</Button>
            </div>
          </motion.div>
        ) : !selectedDoctor ? (
          <motion.div key="search" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="relative mb-3">
              ...
              <input className="input pl-11" placeholder="Search doctors..." value={query} onChange={e => setQuery(e.target.value)} />
            </div>
            <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
              {SPECIALTIES.map(s => (
                <button key={s} onClick={() => setSpecialty(s)} className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition ${specialty === s ? 'bg-primary-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>{s}</button>
              ))}
            </div>
                ...
                <Card key={d.id} onClick={() => d.available && setSelectedDoctor(d)} className={d.available ? '' : 'opacity-50'}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <Chip color={d.available ? 'success' : 'slate'}>{d.available ? 'Available' : 'Unavailable'}</Chip>
                    </div>
          ...
          <motion.div key="book" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            ...
            <p className="font-bold text-slate-800 dark:text-white text-sm mb-2 flex items-center gap-2"><Calendar size={14} /> Select Date</p>
            <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
              {DATES.map(d => (
            ...
            {date && (
              <>
                <p className="font-bold text-slate-800 dark:text-white text-sm mb-2 flex items-center gap-2"><Clock size={14} /> Select Time</p>
                <p className="text-xs text-slate-500 mb-1.5">Morning</p>
                ...
                <p className="text-xs text-slate-500 mb-1.5">Afternoon</p>
            ...
            {slot && (
              <>
                <p className="font-bold text-slate-800 dark:text-white text-sm mb-2 flex items-center gap-2"><Stethoscope size={14} /> Visit Type</p>
                <div className="grid grid-cols-2 gap-3 mb-5">
                  <button onClick={() => setType('inperson')} className={`glass-card p-4 flex flex-col items-center gap-2 transition ${type === 'inperson' ? 'ring-2 ring-primary-500' : ''}`}>
                    ...
                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">In-Person Clinic</span>
                  </button>
                  <button onClick={() => setType('telehealth')} className={`glass-card p-4 flex flex-col items-center gap-2 transition ${type === 'telehealth' ? 'ring-2 ring-primary-500' : ''}`}>
                    ...
                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">Telehealth Video</span>
                  </button>
                </div>
              </>
            )}
