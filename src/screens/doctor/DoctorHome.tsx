...
export function DoctorHome() {
  ...
  return (
    <div className="pb-24 px-4 pt-6 max-w-md mx-auto">
      <PageHeader title="Doctor Dashboard" subtitle="Dr. Michael Chen" right={
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        <Card className="text-center"><div className="w-9 h-9 rounded-xl bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center mx-auto mb-1"><Users size={18} className="text-primary-600" /></div><p className="text-2xl font-extrabold text-slate-800 dark:text-white">{state.doctorPatients.length}</p><p className="text-xs text-slate-500">Active</p></Card>
        <Card className="text-center"><div className="w-9 h-9 rounded-xl bg-danger-100 dark:bg-danger-500/20 flex items-center justify-center mx-auto mb-1"><AlertTriangle size={18} className="text-danger-500" /></div><p className="text-2xl font-extrabold text-danger-500">{flagged}</p><p className="text-xs text-slate-500">Red Alerts</p></Card>
        <Card className="text-center"><div className="w-9 h-9 rounded-xl bg-success-100 dark:bg-success-700/30 flex items-center justify-center mx-auto mb-1"><TrendingUp size={18} className="text-success-600" /></div><p className="text-2xl font-extrabold text-success-600">{avgScore}%</p><p className="text-xs text-slate-500">Avg Score</p></Card>
      </div>

      {/* Active alerts */}
      <p className="font-bold text-slate-800 dark:text-white mb-3">Active Alerts</p>
      ...
      <div className="relative mb-3">
        ...
        <input className="input pl-11" placeholder="Search patients..." value={query} onChange={e => setQuery(e.target.value)} />
      </div>
      <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
        {(['all', 'high', 'moderate', 'low'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap capitalize transition ${filter === f ? 'bg-primary-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>{f} risk</button>
        ))}
      </div>
...
function PatientDrawer({ patientId, onClose }: { patientId: string; onClose: () => void }) {
  ...
  const assignMed = () => {
    if (!med.name) return;
    setState(s => ({ ...s, alerts: [{ id: 'a' + Date.now(), patientId, type: 'Medication', level: 'low', time: 'now', message: `New medication assigned: ${med.name} ${med.dosage}` }, ...s.alerts] }));
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/40 flex items-end sm:items-center justify-center" onClick={onClose}>
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring' }} className="glass-card w-full max-w-md max-h-[90vh] overflow-y-auto rounded-b-none sm:rounded-2xl p-5" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2 mb-4"><RiskBadge level={p.risk} /><Chip color="slate">Last check-in: {p.lastCheckIn}</Chip></div>

        <div className="grid grid-cols-4 gap-2 mb-4">
          {[{ l: 'HR', v: p.recentVitals.heartRate }, { l: 'BP', v: p.recentVitals.bp }, { l: 'Temp', v: p.recentVitals.temp }, { l: 'SpO₂', v: p.recentVitals.oxygen + '%' }].map(x => (
        ...
        <Card className="mb-3"><p className="text-sm font-bold mb-2">Heart Rate Trend</p>
        ...
        <Card className="mb-3"><p className="text-sm font-bold mb-2">Recovery Score</p>
          <ResponsiveContainer width="100%" height={120}>
        ...
        <div className="flex gap-2 mb-3">
          <Button variant="outline" className="flex-1" onClick={() => setShowMed(true)}><Plus size={16} /> Assign Med</Button>
          <Button variant="outline" className="flex-1" onClick={() => { onClose(); navigate('doctor-telehealth'); }}><FileText size={16} /> Message</Button>
        </div>
        <Button variant="ghost" className="w-full" onClick={() => alert('PDF report downloaded (mock)')}><Download size={16} /> Download PDF Report</Button>

        <AnimatePresence>
          {showMed && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-800">
              <p className="font-bold text-sm mb-2">Assign New Medication</p>
              <div className="space-y-2">
                <input className="input" placeholder="Medication name" value={med.name} onChange={e => setMed({ ...med, name: e.target.value })} />
                ...
                <div className="flex gap-2"><Button variant="ghost" className="flex-1" onClick={() => setShowMed(false)}>Cancel</Button><Button className="flex-1" onClick={assignMed}>Assign</Button></div>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
